create view v_addresses
            (id, address, balance, ledger_balance, delegate_id, delegate_address, cliff_amount, cliff_time,
             ...
             voting_for, nonce, tr_count, tr_applied, tr_failed, name, site, telegram, discord, fee, "desc",
...
SELECT b.id,
       ...
       (SELECT count(vt.id) AS count
        FROM v_trans vt
        WHERE vt.source_id = b.id
          AND vt.status = 'applied'::user_command_status)                    AS tr_applied,
       (SELECT count(vt.id) AS count
        FROM v_trans vt
        WHERE vt.source_id = b.id
          AND vt.status = 'failed'::user_command_status)                     AS tr_failed,
       a.name,
       a.site,
       a.telegram,
       a.discord,
...
FROM v_balances b
         ...
         LEFT JOIN v_nonce n ON n.key_id = b.id
